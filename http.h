...
@summary
	A simple to build HTTP library in C.

@todo
- Callback or some other way to get another method in there
	ALWAYS call something after http_set_remaining

- Errors
	(bad handlers)
	(malformed messages are supposed to get status codes)
	(as long as fixed data limits are there, not supported codes are all needed)
	(too large messages should get a status code (doesn't matter where it's handled))
...
- "Stacked" data (headers and POST fields relevant only to a specific key)
	(hash tables...?)

- Multipart POST

- Cookies
  Expire cookies, set cookies etc

- Sessions
	So annoying, but they really belong here

- xfer-encoding: chunked


Less important:
- Manipulation by define
